...
import {queryAllYearData, queryGraduateByYear, queryResearchByYear, queryWorkByYear} from "../../api/api";
       ...
       queryWorkByYear(year).then((res) => {
        ...
        queryAllYearData().then((res) => {
            ...
            for (let year = 2014; year <= 2023; year++) {
                yearList.push(year)
                workList.push(workMap.has(String(year)) ? workMap.get((String(year))) : 0)
                examReList.push(examReMap.has((String(year))) ? examReMap.get((String(year))) : 0)
                exemptList.push(exemptMap.has((String(year))) ? exemptMap.get((String(year))) : 0)
    ...
    return (
        <>
            <Select
                ...
                options={options}
            />
            {option1 && <EChartTemplate style={{
                width: 600,
                height: 400,
                margin: 100
            }} option={option1}/>}

            {option2 && <EChartTemplate style={{
                width: 600,
                height: 400,
                margin: 100
            }} option={option2}/>}

            {option3 && <EChartTemplate style={{
                width: 600,
                height: 400,
                margin: 100
            }} option={option3}/>}
            {option4 && <EChartTemplate style={{
                width: 600,
                height: 400,
                margin: 100
            }} option={option4}/>}
